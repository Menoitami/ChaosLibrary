...
# Путь к CSV-файлу
csv_path = os.path.join(os.path.dirname(__file__), '../results/lle_performance_results.csv')
...
plt.title('LLE Execution Time vs Resolution')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(os.path.dirname(__file__), '../results/lle_time_vs_resolution.png'))
...
plt.ylabel('Execution Time (ms)')
plt.title('LLE Execution Time vs Modeling Time')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(os.path.dirname(__file__), '../results/lle_time_vs_modelingtime.png'))

# --- 3. Execution Time vs NT (если есть) ---
nt_data = data[data['Parameter'] == 'NT']
if not nt_data.empty:
    nt_vals = nt_data['Value'].astype(float)
    
    plt.figure(figsize=(7,5))
    for lib in nt_data['Library'].unique():
        plt.plot(nt_vals[nt_data['Library'] == lib],
                 nt_data[nt_data['Library'] == lib]['ExecutionTime_ms'],
                 marker='o', label=lib)
    plt.xlabel('NT Value')
    plt.ylabel('Execution Time (ms)')
    plt.title('LLE Execution Time vs NT Value')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(os.path.dirname(__file__), '../results/lle_time_vs_nt.png'))

# --- Дополнительная информация ---
def compute_speedup(df, ref_lib='old_library', comp_lib='LLE'):
    speedups = []
    values = []
...
# Вычисляем ускорение для разных тестов, учитывая имена библиотек в данных LLE
libs = data['Library'].unique()
ref_lib = libs[0] if len(libs) > 0 else 'old_library'
comp_lib = libs[1] if len(libs) > 1 else 'LLE'
...
speedup_nt, vals_nt = compute_speedup(nt_data, ref_lib, comp_lib) if not nt_data.empty else ([], [])

print('\n--- Дополнительная информация для LLE ---')
if speedup_res:
...
if speedup_nt:
    print(f'Среднее ускорение {comp_lib} относительно {ref_lib} (NT): {sum(speedup_nt)/len(speedup_nt):.3f}')
    print(f'Минимальное ускорение (NT): {min(speedup_nt):.3f}')
    print(f'Максимальное ускорение (NT): {max(speedup_nt):.3f}')
...
if speedup_res and len(vals_res) > 1:
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(os.path.dirname(__file__), '../results/lle_speedup_vs_resolution.png'))
...
if speedup_mod and len(vals_mod) > 1:
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(os.path.dirname(__file__), '../results/lle_speedup_vs_modelingtime.png'))
