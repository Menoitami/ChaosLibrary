	...
	const int amountOfPoints, double* outPeaks, double* timeOfPeaks, double* valleys, double* TimeOfValleys, double h)
{
	...
	for (int i = startDataIndex + 1; i < startDataIndex + amountOfPoints - 1; ++i)
	{
		...
		if (data[i] - data[i - 1] > 1e-9 && data[i] >= data[i + 1] && data[i] > 0.11)
		{
			...
			for (int j = i; j < startDataIndex + amountOfPoints - 1; ++j)
			{
				...
				if (data[j] - data[j + 1] > 1e-9)
				{
					...
					// --- Если массик timeOfPeaks не пуст, то делаем запись ---
					if (timeOfPeaks != nullptr)
						timeOfPeaks[startDataIndex + amountOfPeaks] = h * (double)(i - startDataIndex);	// Выбираем индекс посередине между j и i
					++amountOfPeaks;
					i = j + 1; // Потому что следующая точка точно не может быть пиком ( два пика не могут идти подряд )
					break;
				}
		...
		//////	for (int j = i; j < startDataIndex + amountOfPoints - 1; ++j)
		//////	{
		//////		// --- Если наткнулись на точку строго меньше, значит это был не минимум ---
		//////		if (data[j] > data[j + 1])
		//////		{
		//////			i = j + 1;	// --- Обновляем внешний счетчик, чтобы дважды не проходить один и тот же интервал
		//////			break;		// --- Возвращаемся к внешнему циклу
		...
		//////				TimeOfValleys[startDataIndex + amountOfValleys] = h * (double)(i - startDataIndex);	// Выбираем индекс посередине между j и i
		//////			++amountOfValleys;
		//////			i = j + 1; // Потому что следующая точка точно не может быть пиком ( два пика не могут идти подряд )
		//////			break;
		//////		}
	...
	const int idx, const double eps, int* outData)
{
	...
	// ------------------------------------------------------------
	// переписать с учетом того что могут быть импульсы при которых 0 спайков
	for (int i = 0; i < amountOfPeaks; i++) {
	...
	double*			resultArray)
{
	...
	// --- НЕ ЗАБУДЬ УБРАТЬ -2, это перерасчет для модели нейрона AND-TS
	for (int i = 0; i < amountOfInitialConditions; ++i)
	{
	...
	for (int i = 0; i < sizeOfBlock; ++i)
	{
		...
		// --- И ЭТО НЕ ЗАБУДЬ УБРАТЬ, это для модели нейрона AND-TS
		//y[2] = x[2];
		//y[3] = x[3];
	}
